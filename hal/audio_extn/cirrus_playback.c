...
static void *audio_extn_cirrus_calibration_thread(void *param);
#else
static void *audio_extn_cirrus_config_thread();
#endif

#ifdef ENABLE_CIRRUS_DETECTION
static void *audio_extn_cirrus_failure_detect_thread(void *param);
#endif
...
#ifdef ENABLE_CIRRUS_DETECTION
void *audio_extn_cirrus_failure_detect_thread(void *param) {
    struct audio_device *adev = handle.adev_handle;
    struct crus_sp_ioctl_header header;
    struct mixer_ctl *ctl = NULL;
